...
public class SmartUpgradeHelper {
	...
	private void initInstance() {
		File folder = new File(ConcreteLogin.getInstance().getDownloadApkRoot());
		if (!folder.exists())
			folder.mkdirs();
...
//			patchHelper = new PatchHelper(mContext);
		...
		downloader = new SmartUpgradeDownloader(info, folder, AppConstants.SAVE_UPGRADE_FILENAME, mThread);
	}
...
//			notification.setLatestEventInfo(mContext, null, null, contentItent);
	...
	private class UpgradeThread extends AsyncTask<Integer, Long, Integer> implements DownloadListener {
		...
		public void onDownloadComplete(String filePath) {
			...
			try {
				if (isNeedPatchAPK) {
					String newApkFilePath =  ConcreteLogin.getInstance().smartPatch(mContext,filePath,fileName);
					//deleteFile(filePath);
					...
					File newApkFile = new File(ConcreteLogin.getInstance().getDownloadApkRoot() + fileName);
					downloadFile.renameTo(newApkFile);
					String md5 = MD5Util.getFileMD5String(newApkFile);
					if (!newApkFile.equals("")) {
