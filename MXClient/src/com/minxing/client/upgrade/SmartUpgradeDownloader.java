...
public class SmartUpgradeDownloader {
	...
	public SmartUpgradeDownloader(AppUpgradeInfo info, File savePath, String saveFileName, DownloadListener downloadListener) {
		this.info = info;
	...
	private void addMXUseAgent(HttpURLConnection conn) {
		...
		if (verName != null && !"".equals(verName)) {
			conn.setRequestProperty("User-Agent", "MinxingMessenger/" + verName);
		} else {
			conn.setRequestProperty("User-Agent", "MinxingMessenger/1.0.0");
		}
	}
