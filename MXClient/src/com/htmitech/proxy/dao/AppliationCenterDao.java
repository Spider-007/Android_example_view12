...
import com.htmitech.proxy.doman.AppVersion;
...
import com.htmitech.proxy.util.DBUnitl;
        ...
        if (db.isOpen()) {
            ...
            try {
                ...
                if (cursor.moveToFirst()) {
                    ...
                    mEmpPortal.apc_style = cursor.getInt(cursor.getColumnIndex("apc_style"));
                    mEmpPortal.font_style = cursor.getInt(cursor.getColumnIndex("font_style"));
                    mEmpPortal.title_display_style = cursor.getInt(cursor.getColumnIndex("title_display_style"));
                    mEmpPortal.mx_appid = cursor.getLong(cursor.getColumnIndex("mx_appid"));
                    mEmpPortal.emi_network_id = cursor.getLong(cursor.getColumnIndex("emi_network_id"));
                    mEmpPortal.network_code = cursor.getString(cursor.getColumnIndex("network_code"));
                    mEmpPortal.group_corp_id = cursor.getLong(cursor.getColumnIndex("group_corp_id"));
                    ...
                    if (mApcUserdefinePortal == null) {
                        ...
                        mApcUserdefinePortal.user_id = BookInit.getInstance().getCrrentUserId() == null ? OAConText.getInstance(HtmitechApplication.instance()).UserID : BookInit.getInstance().getCrrentUserId();
                        BookInit.getInstance().setmApcUserdefinePortal(mApcUserdefinePortal);
                    } else {
...
//        }
        //获取分类应用，防止取自定义表的时候没有分类应用信息
        ArrayList<AppInfo> classifyAppInfo = getClassifyAppInfo();
        //是否需要添加分类应用信息标志位
...
//                        "GROUP BY portal_app.app_id ORDER BY portal_app.appcenter_diplay_order";
                ...
                if(null != classifyAppInfo && classifyAppInfo.size() > 0 && isNeedAdd){
                    for(int i = 0 ; i < classifyAppInfo.size(); i++){
                        appinfosList.add(classifyAppInfo.get(i));
                    }
                }
                while (cursor.moveToNext()) {
                    AppInfo mAppInfo = new AppInfo();
...
//                        }
                    ...
                    if (mAppInfo.getApp_type() == 7 && mAppInfo.getClassifyAppInfo().size() == 0) {
                        ...
                        if (!appinfosList.contains(mAppInfo) && null != mAppVersionList && mAppVersionList.size() > 0) {
...
//                    }
        ...
        ArrayList<AppInfo> tempAppInfo = new ArrayList<>();

        for (AppInfo appInfo : appinfosList) {

            boolean isExist = false;

            for (AppInfo info : appinfosList) {

                if (info.getClassifyAppInfo().contains(appInfo)) {
                    isExist = true;
                    break;
                }
...
//        refresh(appinfosList);
    ...
    * 获取应用中心的所有应用
    * */
    public ArrayList<AppInfo> getAppcenterList(){
        String portal_id = BookInit.getInstance().getPortalId();
        ArrayList<AppInfo> appinfosList = new ArrayList<AppInfo>();
        String sql = "SELECT aag.appgroup_app_id AS app_group_id , aag.display_order AS app_group_display_order ," +
                " portal_app.*, app_info.*, mobile_app_info.current_version FROM portal_app LEFT JOIN mobile_app_info ON" +
                "( portal_app.app_id = mobile_app_info.app_id) LEFT JOIN app_info ON( portal_app.app_id = app_info.app_id)" +
                " LEFT JOIN apc_app_appgroup aag ON (app_info.app_id = aag.app_id AND aag.status_flag > 0) " +
                "LEFT JOIN portal_app gpa ON gpa.portal_id = "+portal_id+" AND gpa.app_id = aag.appgroup_app_id WHERE( app_info.status_flag > 0 " +
                "AND portal_app.status_flag > 0 AND portal_app.appcenter_include = 1 AND portal_app.portal_id = "+portal_id+" ) " +
                "GROUP BY portal_app.app_id ORDER BY portal_app.appcenter_diplay_order";
        if (db.isOpen()) {
            Cursor cursor = null;
            cursor = db.rawQuery(sql, null);
...
//                        }
   ...
   * */
    public ArrayList<AppInfo> getClassifyAppInfo() {
        String portal_id = BookInit.getInstance().getPortalId();
        String sql = "SELECT portal_app.status_flag, aag.appgroup_app_id AS app_group_id , aag.display_order AS app_group_display_order ,\n" +
                " portal_app.*, app_info.* FROM portal_app  \n" +
                " LEFT JOIN app_info ON( portal_app.app_id = app_info.app_id AND app_info.status_flag > 0) \n" +
                " LEFT JOIN (select a.appgroup_app_id ,a.app_id ,display_order  \n" +
                "from  apc_app_appgroup a, portal_app p \n" +
...
//                        }
                ...
                if (mAppInfo.getApp_type() == 7 && mAppInfo.getClassifyAppInfo().size() == 0) {
                    ...
                    if (!appinfosList.contains(mAppInfo) && mAppInfo.getApp_type() == 7) {
                        appinfosList.add(mAppInfo);
                    }
...
//           }
        ...
        sql = "SELECT * FROM portal_app LEFT JOIN app_info on(app_info.app_id = portal_app.app_id)  where portal_app.portal_id = '" + portal_id + "' AND app_info.parent_app_id = '" + appId + "' AND app_info.[status_flag] > 0 AND portal_app.[status_flag] > 0 ORDER BY app_info.default_showinparent_flag  ASC, app_info.app_id DESC\n";
...
//        }
    ...
    public long[] getVersion(int env_type, int version_no) {
        String sql = "";
        if (env_type == 1) {
            sql = "SELECT max(version_no) version_no_max, env_type env_type_max FROM appclient_version WHERE TYPE = 1 AND status_flag > 0 AND env_type=1";
