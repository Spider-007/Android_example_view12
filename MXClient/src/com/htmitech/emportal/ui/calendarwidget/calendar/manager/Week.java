...
public class Week extends RangeUnit {
    ...
    public static int isFristLoop;//是否为第一次循环，用于获取第一个可见天的日期
	private static LocalDate localDate;
	private static LocalDate lastLocalDate;
	...
	public Week(@NonNull LocalDate date, @NonNull LocalDate today, @Nullable LocalDate minDate,
		...
		if(isFristLoop  <= 0){
			localDate = date.withDayOfWeek(1);
			isFristLoop++;
		}
		lastLocalDate = date.withDayOfWeek(7);
		build();
	}
    public static String getFristVisibleDat(){
		if(null != localDate){
			return localDate.toString();
		}
		return "";
	}
	public static String getLastVisibleDat(){
		if(null != localDate){
			return lastLocalDate.toString();
		}
		return "";
	}
	@Override
