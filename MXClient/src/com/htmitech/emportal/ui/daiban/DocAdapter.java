...
public class DocAdapter extends BaseAdapter {
	...
	public DocAdapter(Context context, Boolean isRead) {
		mContext = context;
		isHaveRead = isRead;
	}
	...
	public long getItemId(int position) {
		// TODO Auto-generated method stub
		return 0;
		// return entityList[position].getMessageID();
	}
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		if (convertView == null) {
			...
			holder.imgView  = (ImageView) convertView.findViewById(R.id.itemIcon);
		...
		holder.titleTextView.setText(doc.getDocTitle());
		holder.infoTextView.setText(doc.getDocType());
		holder.timeTextView.setText(isDate(doc.getSendDate()));
...
//						) )
//				holder.imgView.setImageResource(R.drawable.icon_email);
//			else
//				BitmapUtils.instance().display(holder.imgView, doc.getIconId());

			Glide.with(mContext).load(doc.getIconId()).placeholder(R.drawable.icon_email).error(R.drawable.icon_email).transform(new GlideRoundTransform(mContext)).into( holder.imgView);
...
//							doc.getIconId().endsWith(".png") || doc.getIconId().endsWith(".jpg")
//						) )
//				holder.imgView.setImageResource(R.drawable.icon_email_taken);
//			else
//				BitmapUtils.instance().display(holder.imgView, doc.getIconId());
//
			Glide.with(mContext).load(doc.getIconId()).placeholder(R.drawable.icon_email_taken).error(R.drawable.icon_email_taken).transform(new GlideRoundTransform(mContext)).into( holder.imgView);
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		if (doc.getIconId() == null || "".equals(doc.getIconId())) {
			if (isHaveRead) {
				iConImage.setImageResource(R.drawable.icon_email_taken);
			} else {
				iConImage.setImageResource(R.drawable.icon_email);
			}
		}
		...
		if (position != getCount()) {
			if (isHaveRead) {
				BitmapUtils.instance().configDefaultLoadFailedImage(R.drawable.icon_email_taken);
				//iConImage.setImageResource(R.drawable.icon_email_taken);
			} else {
				BitmapUtils.instance().configDefaultLoadFailedImage(R.drawable.icon_email);
				//iConImage.setImageResource(R.drawable.icon_email);
			}
