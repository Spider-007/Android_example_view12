...
public class GeneralFormAdapter extends BaseAdapter {
	...
	private ArrayList<Listiteminfo> entityList = new ArrayList<Listiteminfo>();
	private Context mContext;
	Boolean isHaveRead;
	boolean isHaveBg = false;
	private static int colorSwap = 0;

	public GeneralFormAdapter(Context context, Boolean isRead) {
		mContext = context;
		isHaveRead = isRead;
	}
	...
	public void setData(boolean needClear,  Vector<Listiteminfo> temp) {
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		if (convertView == null) {
			...
			convertView = LayoutInflater.from(mContext).inflate(R.layout.adapter_general_form_todo, null);
		...
		//修改层级背景
		Listiteminfo doc = (Listiteminfo) getItem(position);


		holder.tv_title.setText(doc.getTitle());
		holder.tv_type.setText(doc.getOtherinfo2());
//		holder.tv_time.setText(isDate(doc.getSendDate()));
		holder.tv_time.setText(doc.getTime());
		holder.tv_sendfrom.setText(doc.getOtherinfo1());
        holder.tv_doc.setOnClickListener(myListener);
		if(doc.newtag != null && doc.newtag.equals("1")){
...
//			if (doc.getIconurl() == null || "".equals(doc.getIconurl()))
//				holder.iv_type.setImageResource(R.drawable.icon_email);
//			else
//				BitmapUtils.instance().display(holder.iv_type, doc.getIconurl());
			break;
		case 1:
//			if (doc.getIconurl() == null || "".equals(doc.getIconurl()))
...
//				BitmapUtils.instance().display(holder.iv_type, doc.getIconurl());
	...
	class ViewHolder {
		TextView tv_title ,tv_type ,tv_time,tv_sendfrom,tv_doc,tv_new;
		ImageView iv_type;
	}
    private class MyListener implements View.OnClickListener{
        ...
        public void onClick(View v) {
            // TODO Auto-generated method stub
//            Toast.makeText(HtmitechApplication.instance(), entityList.get(mPosition).getTitle() + ">>>>>>>" + entityList.get(mPosition).getSendFrom(), Toast.LENGTH_SHORT).show();
        }
