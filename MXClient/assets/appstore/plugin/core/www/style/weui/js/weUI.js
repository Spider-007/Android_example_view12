/**
 * Created by xiaoheidepro on 16/10/28.
 */
//标题提醒文字弹层
function dialog(title , msg , callback){
    ...
    dialog1 = '\
            <div class="weui_dialog_confirm" id="dialog1" style="display: none;">\
                <div class="weui_dialog_mask"></div>\
    ...
    dialog2 = '\
            <div class="weui_dialog_alert" id="dialog2" style="display: none;">\
                ...
                <div class="weui_dialog">\
                    <div class="weui_dialog_hd"><strong class="weui_dialog_title">'+ title +'</strong></div>\
                    <div class="weui_dialog_bd" width="80%">' + msg + '</div>\
                    <div class="weui_dialog_ft" >\
                        <a href="javascript:;" class="weui_btn_dialog primary">确定</a>\
                    </div>\
                </div>\
...
function toast(){
    ...
    toast = '\
            <div id="toast" style="display: none;">\
                <div class="weui_dialog_mask_transparent"></div>\
...
function actionSheetMenu(){
    ...
    for(i=0;i<arguments.length;i++){
        ...
        menuHtml += '<div class="weui_dialog_actionsheet_cell" id="' + arguments[i].id + '">' + arguments[i].text + '</div>';
    }
    if(!menuHtml){
        return;
    }
    ...
    actionSheetMenu = '\
            <div id="actionSheet_wrap">\
                <div class="weui_dialog_mask_transition" id="mask"></div>\
                <div class="weui_dialog_actionsheet" id="weui_dialog_actionsheet">\
                    <div class="weui_dialog_actionsheet_menu">\
                    </div>\
                    <div class="weui_dialog_actionsheet_action">\
                        <div class="weui_dialog_actionsheet_cell" id="actionsheet_cancel">取消</div>\
                    </div>\
                </div>\
            </div>\
        ';
    ...
    $('.weui_dialog_actionsheet_menu').html(menuHtml);
    $('#mask').show().addClass('weui_dialog_fade_toggle');
    $('#weui_dialog_actionsheet').addClass('weui_dialog_actionsheet_toggle');
    $('#mask').unbind('transitionend').unbind('webkitTransitionEnd');
    $('#mask , #actionsheet_cancel').on('click',function(){
        $('#mask').removeClass('weui_dialog_fade_toggle');
        $('#weui_dialog_actionsheet').removeClass('weui_dialog_actionsheet_toggle');
