...
public class SoundAnnotDialog implements KinggridConstant,OnClickListener {
	 ...
	 * @param annotation
	 */
	public SoundAnnotDialog(final IAppPDFActivity activity, final int width,
                            final int height, final Annotation annotation) {
	...
	private void initView() {
		...
		popupWindow = new PopupWindow(view, (int) (400 * IAppPDFActivity.densityCoefficient),
	...
	private void showAlert(View parent, boolean playFlag) {
		...
		alertWindow = new PopupWindow(alertView, (int)(594 * IAppPDFActivity.densityCoefficient), (int)(214 * IAppPDFActivity.densityCoefficient));
//				android.view.ViewGroup.LayoutParams.WRAP_CONTENT);
		alertWindow.setFocusable(true);
		alertWindow.setOutsideTouchable(false);

		alertWindow.showAtLocation(parent, Gravity.CENTER_HORIZONTAL, 0, (int) (-75 * IAppPDFActivity.densityCoefficient));
		alertWindow.update();
	}
...
//									+ rawData.length);
				...
				popupWindow.showAtLocation(view, Gravity.CENTER, offsetX,
						offsetY);
				popupWindow.update();
			}
	}
	...
	public void onClick(View v) {
		int i1 = v.getId();
		if (i1 == R.id.btn_record) {
			startVoiceT = System.currentTimeMillis();
			fileName = IAppPDFActivity.userName + startVoiceT + ".raw";
		...
		} else if (i1 == R.id.btn_pause) {
			mBtnPlay.setVisibility(View.VISIBLE);
			mBtnPause.setVisibility(View.GONE);
			pauseSound();

		} else if (i1 == R.id.btn_delete) {
		...
		} else if (i1 == R.id.btn_close_annot) {
			isSaved = false;
			if (isPlay || isPause) {
				playSound.stop();
			}
