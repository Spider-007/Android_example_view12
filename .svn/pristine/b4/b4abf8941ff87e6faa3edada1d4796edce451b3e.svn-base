...
public class ZBWaterDropListViewHeader extends FrameLayout {
    private LinearLayout mContainer;
    private ProgressBar mProgressBar;
    private ZBWaterDropView mWaterDropView;
    ...
    public ZBWaterDropListViewHeader(Context context) {
        super(context);
        initView(context);
    }
     ...
     * @param attrs
     */
    public ZBWaterDropListViewHeader(Context context, AttributeSet attrs) {
        super(context, attrs);
        initView(context);
    }
    ...
    private void initView(Context context) {
        mContainer = (LinearLayout) LayoutInflater.from(context).inflate(
                R.layout.zb_waterdroplistview_header, null);
        mProgressBar = (ProgressBar) mContainer.findViewById(R.id.waterdroplist_header_progressbar);
        mWaterDropView = (ZBWaterDropView) mContainer.findViewById(R.id.waterdroplist_waterdrop);
        // 初始情况，设置下拉刷新view高度�?0
    ...
    public void setVisiableHeight(int height) {
        ...
        if(mState == STATE.stretch){
            float pullOffset = (float) ZBUtils.mapValueFromRangeToRange(height, stretchHeight, readyHeight, 0, 1);
