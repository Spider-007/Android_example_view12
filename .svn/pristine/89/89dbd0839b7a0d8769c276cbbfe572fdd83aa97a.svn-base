...
public class OutlineAdapter extends BaseAdapter {
    ...
    public OutlineAdapter(final Context context, final IAppPDFActivity base, final List<OutlineLink> objects,
                          final OutlineLink current) {

        this.context = context;
        
        final BookSettings bs = base.getController().getBookSettings();
        final DocumentModel model = base.getController().getDocumentModel();
        final Resources resources = context.getResources();

        //background = resources.getDrawable(R.drawable.kinggrid_viewer_outline_item_background);
        //selected = resources.getDrawable(R.drawable.kinggrid_viewer_outline_item_background_selected);
        background = resources.getColor(android.R.color.background_light);
        selected = resources.getColor(android.R.color.background_dark);
        ...
        for (int i = 0; i < this.objects.length; i++) {
            ...
            if (next < this.objects.length && this.objects[i].level < this.objects[next].level) {
                ...
                states[i] = OutlineItemState.EXPANDED;
                rebuild();
                treeFound = true;
            } else {
    ...
    private int getParentId(final int id) {
    ...
    public View getView(final int position, final View convertView, final ViewGroup parent) {
        ...
        btn.setTag(position);
        pageIndex.setText("第"+ getPageIndex(position) + "页");


        //container.setBackgroundDrawable(id == currentId ? this.selected : this.background);
        container.setBackgroundColor(id == currentId ? this.selected : this.background);

        if (firstTime) {
            final LayoutParams btnParams = (LayoutParams) btn.getLayoutParams();
            spaceWidth = btnParams.width / 2;
            //container.setOnClickListener(voidListener);
            //view.setOnClickListener(itemListener);
        }

        final LayoutParams sl = (LayoutParams) space.getLayoutParams();
        ...
        if (states[id] == OutlineItemState.LEAF) {
            ...
            btn.setBackgroundResource(states[id] == OutlineItemState.EXPANDED ? R.drawable.kinggrid_outline_list_expand
                        : R.drawable.kinggrid_outline_list_collapse);
        }
