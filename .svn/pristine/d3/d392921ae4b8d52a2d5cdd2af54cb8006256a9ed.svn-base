...
/**
 * SlidingLayout provides a horizontal, multi-pane layout for use at the top level of a UI. A left (or first) pane is
 * treated as a content list or browser, subordinate to a primary detail view for displaying content.
 * 
 * <p>
 * Child views may overlap if their combined width exceeds the available width in the SlidingLayout. When this occurs
 * the user may slide the topmost view out of the way by dragging it, or by navigating in the direction of the
 * overlapped view using a keyboard. If the content of the dragged child view is itself horizontally scrollable, the
 * user may grab it by the very edge.
 * </p>
 * 
 * <p>
 * Thanks to this sliding behavior, SlidingLayout may be suitable for creating layouts that can smoothly adapt across
 * many different screen sizes, expanding out fully on larger screens and collapsing on smaller screens.
 * </p>
 * 
 * <p>
 * SlidingLayout is distinct from a navigation drawer as described in the design guide and should not be used in the
 * same scenarios. SlidingLayout should be thought of only as a way to allow a two-pane layout normally used on larger
 * screens to adapt to smaller screens in a natural way. The interaction patterns expressed by SlidingLayout imply a
 * physicality and direct information hierarchy between panes that does not necessarily exist in a scenario where a
 * navigation drawer should be used instead.
 * </p>
 * 
 * <p>
 * Appropriate uses of SlidingLayout include pairings of panes such as a contact list and subordinate interactions with
 * those contacts, or an email thread list with the content pane displaying the contents of the selected thread.
 * Inappropriate uses of SlidingLayout include switching between disparate functions of your app, such as jumping from a
 * social stream view to a view of your personal profile - cases such as this should use the navigation drawer pattern
 * instead. ( {@link android.support.v4.widget.DrawerLayout DrawerLayout} implements this pattern.)
 * </p>
 * 
 * <p>
 * Like {@link android.widget.LinearLayout LinearLayout}, SlidingLayout supports the use of the layout parameter
 * <code>layout_weight</code> on child views to determine how to divide leftover space after measurement is complete. It
 * is only relevant for width. When views do not overlap weight behaves as it does in a LinearLayout.
 * </p>
 * 
 * <p>
 * When views do overlap, weight on a slideable pane indicates that the pane should be sized to fill all available space
 * in the closed state. Weight on a pane that becomes covered indicates that the pane should be sized to fill all
 * available space except a small minimum strip that the user may use to grab the slideable view and pull it back over
 * into a closed state.
 * </p>
 */
public class SlidingLayout extends ViewGroup {
    ...
    /**
     * Default size of the overhang for a pane in the open state. At least this much of a sliding pane will remain
     * visible. This indicates that there is more content available and provides a "physical" edge to grab to pull it
     * closed.
     */
    ...
    /**
     * The size of the overhang in pixels. This is the minimum section of the sliding panel that will be visible in the
     * open state to allow for a closing drag.
     */
    ...
    /**
     * Stores whether or not the pane was open the last time it was slideable. If open/close operations are invoked this
     * state is modified. Used by instance state save/restore.
     */
    ...
    public interface PanelSlideListener {
        ...
        /**
         * Called when a sliding pane becomes slid completely open. The pane may or may not be interactive at this point
         * depending on how much of the pane is visible.
         * 
        ...
        /**
         * Called when a sliding pane becomes slid completely closed. The pane is now guaranteed to be interactive. It
         * may now obscure other views in the layout.
         * 
    ...
    /**
     * No-op stubs for {@link SlidingLayout.PanelSlideListener}. If you only want to implement a subset of the listener
    ...
    /**
     * Set a distance to parallax the lower pane by when the upper pane is in its fully closed state. The lower pane
     * will scroll between this position and its fully open state.
     * 
    ...
    /**
     * Set the color used to fade the pane covered by the sliding pane out when the pane will become fully covered in
     * the closed state.
     * 
     * @param color An ARGB-packed color value
     */
                ...
                if (canSlide && child != mSlideableView) {
                    if (lp.width < 0 && (measuredWidth > fixedPanelWidthLimit || lp.weight > 0)) {
                        ...
                        final int childWidthSpec =
                                MeasureSpec.makeMeasureSpec(fixedPanelWidthLimit, MeasureSpec.EXACTLY);
                        child.measure(childWidthSpec, childHeightSpec);
                    }
                } else if (lp.weight > 0) {
                    int childHeightSpec;
                    ...
                    if (canSlide) {
                        ...
                        final int childWidthSpec =
                                MeasureSpec.makeMeasureSpec(measuredWidth + addedWidth, MeasureSpec.EXACTLY);
                        child.measure(childWidthSpec, childHeightSpec);
                    }
                }
            }
    ...
    public boolean onInterceptTouchEvent(MotionEvent ev) {
        ...
        try {// 淇����ArrayIndexOutOfBoundsException���bug
    ...
    /**
     * Check if the layout is completely open. It can be open either because the slider itself is open revealing the
     * left pane, or if all content fits without sliding.
     * 
    ...
    /**
     * Check if the content in this layout cannot fully fit side by side and therefore the content pane can be slid back
     * and forth.
     * 
     * @return true if content in this layout can be slid open and closed
     */
    ...
    protected boolean drawChild(Canvas canvas, View child, long drawingTime) {
        ...
        if (Build.VERSION.SDK_INT >= 11) { // HC
            ...
            if (lp.dimWhenOffset && mSlideOffset > 0) {
                ...
                try {
                    // 此处getDrawingCache在某些系统上会出现crash，buildDrawingCache
    ...
    private class DragHelperCallback extends ViewDragHelper.Callback {
        ...
        public void onViewReleased(View releasedChild, float xvel, float yvel) {
            ...
            if (xvel > 0 || (xvel == 0 && mSlideOffset > 0.3f)) {
                left += mSlideRange;
            }
    ...
    static class SlidingPanelLayoutImplJB extends SlidingPanelLayoutImplBase {
         ...
         * 
         * In Jellybean, some optimizations in the hardware UI renderer prevent a changed Paint on a View using a
         * hardware layer from having the intended effect. This twiddles some internal bits on the view to force it to
