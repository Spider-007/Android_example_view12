...
public class SystemDiagnosisActivity extends RootActivity {
	...
	private void refreshServerConfigData() {
		diagosis_http_server_tv.setText(ConcreteLogin.getInstance().getServerHost());
		diagosis_http_server_et.setText(ConcreteLogin.getInstance().getServerHost());
...
//						MXKit.getInstance().getKitConfiguration().switchHttpServerConfig(SystemDiagnosisActivity.this, newHttpServerConfig);
						ConcreteLogin.getInstance().switchHttpServerConfig(SystemDiagnosisActivity.this, newHttpServerConfig);
					}
				}
				refreshServerConfigData();
			}
		...
		diagosis_push_server_tv.setText(ConcreteLogin.getInstance().getPushHost());
		diagosis_push_server_et.setText(ConcreteLogin.getInstance().getPushHost());
			...
			public void onClick(View v) {
				if (diagosis_push_server_tv.getVisibility() == View.VISIBLE) {
					...
					if (newPushServerConfig != null && !"".equals(newPushServerConfig)) {
						...
						ConcreteLogin.getInstance().switchPushServerConfig(SystemDiagnosisActivity.this, newPushServerConfig);
					}
				}
				refreshServerConfigData();
			}
		...
		new Thread(new Runnable() {
			...
			public void run() {
				try {
					...
					// 生成SSLSocket
					String pushHost = ConcreteLogin.getInstance().getPushHost();
					if (pushHost != null && !"".equals(pushHost)) {
						...
						if (pushIPPort.length == 2) {
							...
							if (socket != null && socket.isConnected()) {
								final MqttClient client = new MqttClient(ConcreteLogin.getInstance().getPushHost(),
								...
								SSLContext mqttSslContext = SSLContext.getInstance("TLS");
