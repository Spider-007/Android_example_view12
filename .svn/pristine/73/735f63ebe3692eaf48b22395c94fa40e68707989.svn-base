...
public class MyRouteBaseAdapter extends BaseAdapter {
	...
	Vector<RouteInfo> vector = new Vector<RouteInfo>();
	Context context;
	HashMap<Integer, Boolean> mcheckhm = new HashMap<Integer, Boolean>();

	public MyRouteBaseAdapter(Context context, Vector<RouteInfo> temp) {
	...
	public void setData(Vector<RouteInfo> temp) {
		vector.clear();
		vector.addAll(temp);
		mcheckhm.clear();
		this.notifyDataSetChanged();
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		RouteInfo info = vector.elementAt(position);
		holder.cb.setChecked(mcheckhm.get(position) == null ? false : mcheckhm
				.get(position));
		holder.textview_name.setText(info.getRouteName());
		return convertView;
	}
	...
	// public RsSimpleAdapter(Context context,
	// List<? extends Map<String, ?>> data, int resource, String[] from,
	// int[] to, int restobj, boolean issettag, RouteInfo[] dt) {
	// super(context, data, resource, from, to);
	// mrestobj = restobj;
	// mData = data;// list数据
	// mResource = resource;
