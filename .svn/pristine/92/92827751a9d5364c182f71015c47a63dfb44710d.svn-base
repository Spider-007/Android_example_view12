...
public class YAxisRender extends AxisRender {
    ...
    public void drawGraph(Canvas canvas) {
        ...
        for (int i=0;(yAxisData.getInterval()*i+yAxisData.getMinimum())<=yAxisData.getMaximum()+yAxisData.getInterval();i++){
            canvas.save();
            canvas.scale(1,-1);
            /**
             * 坐标数值
            ...
            float TextPathY = (mPaintText.descent()+mPaintText.ascent())/2+(float) (yAxisData.getInterval()*i*yAxisData.getAxisScale());
            mPoint.x =-TextPathX;
            mPoint.y =-TextPathY;
            textCenter(new String[]{numberFormat.format(yAxisData.getInterval()*i+yAxisData.getMinimum())},
                    mPaintText,canvas, mPoint, Paint.Align.RIGHT);
//            if (i>0)
//            canvas.drawLine(0,-TextPathY,xAxisData.getAxisLength(),-TextPathY,linePaint);
        ...
        canvas.save();
        canvas.scale(1,-1);
        canvas.drawText(yAxisData.getUnit(),0,-yAxisData.getAxisLength()+(mPaintText.descent()+mPaintText.ascent())*2,mPaintText);
        canvas.restore();
    }
}
