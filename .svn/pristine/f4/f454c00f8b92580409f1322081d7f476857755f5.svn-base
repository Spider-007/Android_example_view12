...
public class CommonFromListAdapter extends BaseAdapter {


	private ArrayList<CommonFormResult> entityList = new ArrayList<CommonFormResult>();
	private Context mContext;
	Boolean isHaveRead;
	boolean isHaveBg = false;
	private static int colorSwap = 0;

	public CommonFromListAdapter(Context context, Boolean isRead) {
		mContext = context;
		isHaveRead = isRead;
	}
	...
	public void setData(boolean needClear,  Vector<CommonFormResult> temp) {
		if (needClear)
			entityList.clear();
		for (CommonFormResult entity : temp) {
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		if (convertView == null) {
			...
			convertView = LayoutInflater.from(mContext).inflate(R.layout.todo_comonform_list_item3, null);
		...
		//修改层级背景
		CommonFormResult commonFormResult = (CommonFormResult) getItem(position);
		holder.tv_title.setText(commonFormResult.getTitle());
		holder.tv_type.setText(commonFormResult.getTime());
//		holder.tv_time.setText(isDate(doc.getSendDate()));
		holder.tv_time.setText(commonFormResult.getOtherinfo2());
		holder.tv_sendfrom.setText(commonFormResult.getOtherinfo1());
		...
		switch (type) {
		case 0:
			if (commonFormResult.getIconurl() == null || "".equals(commonFormResult.getIconurl())
					|| !( 
							commonFormResult.getIconurl().endsWith(".png") || commonFormResult.getIconurl().endsWith(".jpg")
						) ) 
				holder.iv_type.setImageResource(R.drawable.icon_email);
			else
				BitmapUtils.instance().display(holder.iv_type, commonFormResult.getIconurl());
			break;
		case 1:
			if (commonFormResult.getIconurl() == null || "".equals(commonFormResult.getIconurl())
					|| !( 
    ...
    private class MyListener implements View.OnClickListener{
        ...
        public void onClick(View v) {
            // TODO Auto-generated method stub
            Toast.makeText(HtmitechApplication.instance(), entityList.get(mPosition).getTitle() + ">>>>>>>" + entityList.get(mPosition).getForm_id(), Toast.LENGTH_SHORT).show();
        }
    }
