...
/**
 * Created by Idtk on 2016/6/6. Blog : http://www.idtkm.com GitHub :
 * https://github.com/Idtk 描述 : 坐标轴计算帮助类
 */
public class Compute {
	 ...
	 *            第几组数据
	 */
	protected void initMaxMin(float max, float min, int count,
			IAxisData axisData) {
		...
		if (max > 1) {
			...
			min = (float) (Math.floor(min / Math.pow(10, number)
					* Math.pow(10, number)));
		} else {
			while (0 < max && max < 1) {
				max = max * 10;
		...
		if (count == 0) {
			...
			axisData.setMinimum(axisData.getMinimum() > min ? min : axisData
					.getMinimum());
			axisData.setMaximum(axisData.getMaximum() < max ? max : axisData
					.getMaximum());
		}
		minAxisSgin = 1;
		maxAxisSgin = 1;
	 ...
	 *            数据长度
	 */
	protected void initScaling(float min, float max, int length,
			IAxisData axisData) {
		float scaling;
		int count = 0;
		/**
		 ...
		 */
		if (length < 16 && length != 0 && length != 1) {
			scaling = (max - min) / (length - 1);
			// LogUtil.d("TAG",max+":"+min+":"+scaling+":"+length);
		} else if(length < 16 && length != 0 && length != 1) {
			scaling = (max - min);
		}else {
			scaling = (max - min) / 15;
		}
