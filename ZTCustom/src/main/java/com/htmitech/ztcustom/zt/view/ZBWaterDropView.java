...
import com.htmitech.ztcustom.zt.domain.ZBCircle;
import com.htmitech.ztcustom.zt.util.ZBUtils;


@TargetApi(Build.VERSION_CODES.HONEYCOMB)
@SuppressLint("NewApi")
public class ZBWaterDropView extends View {

    private ZBCircle topCircle;
    private ZBCircle bottomCircle;

    private Paint mPaint;
    private Path mPath;
    private float mMaxCircleRadius;//ֵ
    private float mMinCircleRaidus;//ֵ
    private Bitmap arrowBitmap;//
    private final static int BACK_ANIM_DURATION = 180;
    private final static float STROKE_WIDTH = 2;

    public ZBWaterDropView(Context context) {
        super(context);
        init(context, null);
    }

    public ZBWaterDropView(Context context, AttributeSet attrs) {
        super(context, attrs);
        init(context, attrs);
    }

    public ZBWaterDropView(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    private void parseAttrs(Context context, AttributeSet attrs) {
        if (attrs != null) {
            TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.ZBWaterDropView, 0, 0);
            try {
                ...
                if(a.hasValue(R.styleable.ZBWaterDropView_waterdrop_color)){
                    int waterDropColor =  a.getColor(R.styleable.ZBWaterDropView_waterdrop_color, Color.GRAY);
                    mPaint.setColor(waterDropColor);
                }
                if (a.hasValue(R.styleable.ZBWaterDropView_max_circle_radius)) {
                    mMaxCircleRadius = a.getDimensionPixelSize(R.styleable.ZBWaterDropView_max_circle_radius, 0);
                ...
                if (a.hasValue(R.styleable.ZBWaterDropView_min_circle_radius)) {
                    mMinCircleRaidus = a.getDimensionPixelSize(R.styleable.ZBWaterDropView_min_circle_radius, 0);
    ...
    private void init(Context context, AttributeSet attrs) {
        topCircle = new ZBCircle();
        bottomCircle = new ZBCircle();
        mPath = new Path();
        mPaint = new Paint();
        ...
        arrowBitmap = ZBUtils.drawableToBitmap(drawable);
...
//        canvas.drawBitmap(arrowBitmap, topCircle.getX() - topCircle.getRadius(), topCircle.getY() - topCircle.getRadius(), mPaint);
    ...
    public ZBCircle getTopCircle() {
        return topCircle;
    }

    public ZBCircle getBottomCircle() {
        return bottomCircle;
    }
