 ...
 *******************************************************************************/
package com.htmitech.commonx.pulltorefresh.library;
...
import com.htmitech.commonx.pulltorefresh.library.internal.FlipLoadingLayout;
import com.htmitech.commonx.pulltorefresh.library.internal.LoadingLayout;
import com.htmitech.commonx.pulltorefresh.library.internal.RotateLoadingLayout;
import com.htmitech.commonx.pulltorefresh.library.internal.Utils;
import com.htmitech.commonx.pulltorefresh.library.internal.ViewCompat;
...
public abstract class PullToRefreshBase<T extends View> extends LinearLayout implements IPullToRefresh<T> {
	...
	private void init(Context context, AttributeSet attrs) {
		...
		// Styleables from XML
		TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.mx_PullToRefresh);

		if (a.hasValue(R.styleable.mx_PullToRefresh_mx_ptrMode)) {
			mMode = Mode.mapIntToValue(a.getInteger(R.styleable.mx_PullToRefresh_mx_ptrMode, 0));
		}

		if (a.hasValue(R.styleable.mx_PullToRefresh_mx_ptrAnimationStyle)) {
			mLoadingAnimationStyle = AnimationStyle.mapIntToValue(a.getInteger(
					R.styleable.mx_PullToRefresh_mx_ptrAnimationStyle, 0));
		}
		 ...
		 * Styleables from XML
		 */
		if (a.hasValue(R.styleable.mx_PullToRefresh_mx_ptrRefreshableViewBackground)) {
			Drawable background = a.getDrawable(R.styleable.mx_PullToRefresh_mx_ptrRefreshableViewBackground);
			if (null != background) {
				mRefreshableView.setBackgroundDrawable(background);
			}
		} else if (a.hasValue(R.styleable.mx_PullToRefresh_mx_ptrAdapterViewBackground)) {
			Utils.warnDeprecation("ptrAdapterViewBackground", "ptrRefreshableViewBackground");
			Drawable background = a.getDrawable(R.styleable.mx_PullToRefresh_mx_ptrAdapterViewBackground);
			if (null != background) {
				mRefreshableView.setBackgroundDrawable(background);
			}
		}

		if (a.hasValue(R.styleable.mx_PullToRefresh_mx_ptrOverScroll)) {
			mOverScrollEnabled = a.getBoolean(R.styleable.mx_PullToRefresh_mx_ptrOverScroll, true);
		}

		if (a.hasValue(R.styleable.mx_PullToRefresh_mx_ptrScrollingWhileRefreshingEnabled)) {
			mScrollingWhileRefreshingEnabled = a.getBoolean(
					R.styleable.mx_PullToRefresh_mx_ptrScrollingWhileRefreshingEnabled, false);
		}
